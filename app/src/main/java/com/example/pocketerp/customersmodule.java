...
public class customersmodule extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_customersmodule);
    ...
    public void insert() {
            ...
            if (name.equals("")) {
                Toast.makeText(getApplicationContext(), "Please Enter Customer Name to Save", Toast.LENGTH_SHORT).show();
            }
            else {

                String selectQuery=String.format("SELECT * from customerinfo where name='%s'",name);
                ...
                try {
                    ...
                    if (cursor.getCount() > 0) {
                        Toast.makeText(getApplicationContext(), "This Customer Is Already Added", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(getApplicationContext(), "New", Toast.LENGTH_SHORT).show();
                    String sql = "insert into customerinfo(name) values(?)";
                    SQLiteStatement statements = db.compileStatement(sql);
                    statements.bindString(1, name);
                    statements.execute();
                    Toast.makeText(this, "Customers Added", Toast.LENGTH_SHORT).show();
                    ed1.setText("");
    ...
    public void display() {
        try {
            ...
            final Cursor c = db.rawQuery("select * from customerinfo", null);
            ...
            final ArrayList<customerclass> stud = new ArrayList<customerclass>();

            if (c.moveToFirst()) {
                do {

                    customerclass stu = new customerclass();
    ...
    public void delete() {
        try {
            ...
            String name = ed1.getText().toString();
            if(name.equals("")) {
                Toast.makeText(getApplicationContext(), "Please Select Any Customer to Delete", Toast.LENGTH_SHORT).show();
            }
            else{
    ...
    public void edit() {
        try {
            ...
            if(name.equals("")) {
                Toast.makeText(getApplicationContext(), "Please Select Any Customer to Update", Toast.LENGTH_SHORT).show();
                ...
                String sql = "update customerinfo set name=? where id=?";
                SQLiteStatement statements = db.compileStatement(sql);
                statements.bindString(1, name);
    ...
    public int newnumber() {
        ...
        String selectQuery = "SELECT max(id) as id FROM customerinfo";
        SQLiteDatabase db = openOrCreateDatabase("pocketerpsqlitedbnew", Context.MODE_PRIVATE, null);
        db.execSQL("CREATE TABLE IF NOT EXISTS customerinfo(id integer primary key autoincrement,name varchar)");
        Cursor cursor = db.rawQuery(selectQuery, null);
